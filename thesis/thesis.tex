...
under auspices of the Linux Foundation\cite{OCIabout}. The \gls{oci}'s purpose is the standardization of Docker's container image specification (image-spec) and container runtime (runtime-spec).
Before the \gls{oci} there has been many different container image and runtime implementations such like systemd-nspawn and LCX (Linux Containers). The \gls{oci} tries to
address this problem by proposing a standard for a user experience and \gls{api} specifications. This standardization process is very important for operating in different environments
or domains. For example, \gls{oci} image specification allows to run an \gls{oci} compliant virtual machine via an \gls{oci} compliant container runtime.
