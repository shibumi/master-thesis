...
SHA256 checksum over the binary content of the layer for ensuring integrity and reproducibility. This minor information
...
\autoref{lst:imageManifest} depicts a very simplified example image manifest. The sha256 checksums have been shortened for saving space in this document.
...
size and a digest (a checksum) of the addressable content. Annotations provide further information for the image. The OCI manifest specification
...
providing information for various other use cases, like easier search in an OCI compatible registry or information about its maintainers.
With the manifest it is possible to address the content via its offset and to provide integrity via digests. One or more image manifests can form
an image index. An image index is a set of different image manifests for different operating systems and architectures. \autoref{imageIndex} illustrates
