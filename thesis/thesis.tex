...
isolation environment, called "jails", in 2000\cite{FreeBSD4Announcement}. Jails is heavily being influenced by the chroot system call on Unix systems.
...
supervision of the Linux Foundation, has been founded in 2015, and is called \glsfirst{cncf}\cite{CNCFFounding}.
...
this workflow has changed. Docker has introduced a unique file format for describing a container consisting of different layers. These layers
are stacked on each other with union file systems for a final Docker image. This process has the advantage that each layer can be cached separately, leading
to less disk usage and faster container build times.
