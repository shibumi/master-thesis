...
is the use of a hypervisor (how this is implemented and and more can be found in \autoref{section:workload_partitioning_strategies}). 
...
Effectors and sensors communicate with the flight cabinet over \gls{arinc629}\cite{prisaznuk1992integrated}, hence these devices must be \gls{arinc629} compatible.
...
or connected to the global data bus, for examplev via remote data concentrators. \autoref{fig:ima} depicts a simplified view on integrated modular avionics architecture in a plane.
...
with the data bus standard and the hosted functions in the flight cabinets.
...
the system that controls the flight entertainment system is less important than the system that controls the cabin air supply or the temperature. Therefore, every hosted function needs a
priority level.
