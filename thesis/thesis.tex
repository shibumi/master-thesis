...
has been implemented as distributed collection of dedicated computing resources consisting of \gls{lru} or \gls{lrm}\cite{watkins2007transitioning}.
\gls{lru} and \gls{lrm} are modular components that are specifically designed for pre-determined tasks, such as
...
Each \gls{lru} or \gls{lrm} contained one specific avionic workload and its required computing resources (processors, 
\gls{io} modules, main memory, hard disks and network cards). \autoref{fig:federated_architecture} shows a simplified model
of the federated architecture with distributed \gls{lru}, sensors, effectors, and a global data bus connecting the components.
...
system reliability\cite{prisaznuk1992integrated}, but duplicating also means duplicating the \gls{lru} as a whole. This does not
only mean a duplicated hosted function (the actually functionality of the \gls{lru}), it also means double as much cables,
processors, main memory, network cards and connectors.
