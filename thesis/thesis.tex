...
\setcounter{biburlucpenalty}{9001}
...
Automation protects against human errors and is a foundation for all other \gls{slsa} levels. Attaching provenance or a \glsfirst{sbom}  has several advantages. First, it allows to store metadata about the dependencies and the software itself.
...
reproducible builds via providing an isolated and ephemeral build environment. Interestingly, Cloud-Native containers are not that popular in the aviation industry. This is astonishing, because containers have, like previous chapters have showed,
...
The left structure in \autoref{fig:vxworks_kubernetes} shows a diagram of the RT-CASE container engine for running real-time containers in a mixed-criticality system. The RT-CASE engine is a prototype for a 
...
shows the same scenario with all tasks running under the \emph{SCHED\_RR} policy. \emph{SCHED\_RR} defines a fixed time slice for each task. Although the scheduler sets the fixed time slice for each task to one, task A is served
...
\autoref{fig:memory_pages} depicts different memory pages. Processes running in user mode have a restricted permission set. The user page table on the left in \autoref{fig:memory_pages} depicts this scenario.
...
directly in kernel space and therefore hidden for other processes running in user mode.
\begin{figure}[H]
  \centering
...
has multiple advantages for time-sensitive systems. First of all, if a process is not scheduled to a dedicated \gls{cpu} the normal kernel scheduler takes over and schedules the process for best multi \gls{cpu} utilization
...
and quotas enable the system to set a hard cap of \gls{cpu} time over a period (handled by the \emph{limits} statement in Kubernetes)\cite{KubernetesCPUManager}.
...
In this scenario, we will start the pod as defined in the definition \emph{limit\_test.yaml} on the cluster, then invoke a fork bomb to provoke an out-of-memory situation and then monitor what happens.
...
One deployment with \gls{qos} class \emph{Guaranteed} runs already on the cluster. What will happen if we try to schedule another deployment with 4 \glspl{cpu} and 8000MB memory?
...
\begin{figure}[H]
  \centering
  \includegraphics[width=1.0\textwidth]{figures/htop_cpu_manager.png}
  \caption{htop output with CPU manager policy set to static}\label{fig:htop_cpu_manager}
\end{figure}
\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{figures/scenario3.png}
...
with a custom \glsfirst{cni} plugin called \emph{calico}: \emph{minikube start --cpus 4 --memory 4096 --vm --cni=calico}. \emph{Calico} is needed, because the default KubeProxy components provide no network isolation
...
Line 6 has an empty pod selector, this means that the network policy will apply to all pods. Line 7-9 describes the policy types: ingress and egress. Line 10 sets the ingress policy to an empty list, blocking all incoming traffic.
...
a full overview to Kubernetes. Kubernetes is much larger than introduced in this thesis and grows rapidly due to massive contributions by big global players like Google, Intel, IBM, Microsoft or Amazon.
